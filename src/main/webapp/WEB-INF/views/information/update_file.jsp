...
<c:set var="thumb1" value="${informationVO.thumb1.toLowerCase() }" />
...
<c:import url="/menu/top.do" />
  <DIV class='title_line'> ${sectVO.name } > ${title } >파일 수정</DIV>
    ...
    <form name='frm' id='frm' method='get' action='./list_by_sectno_search_paging.do'>
      ...
      <c:if test="${param.word.length() > 0 }">
        <button type='button' class='btn btn-secondary btn-sm' 
                    onclick="location.href='./list_by_sectno.do?sectno=${sectVO.sectno}&word='" style="padding: 2px 8px 3px 8px; margin: 0px 0px 2px 0px;">검색 취소</button>  
      </c:if>    
    </form>
  </div>
      ...
      <li class="li_none">
        <DIV style='text-align: center; width: 50%; float: left;'>
          <c:choose>
            <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}">
              <IMG src="/information/storage/${file1saved }" style='width: 90%;'> 
            </c:when>
            <c:otherwise> <!-- 이미지가 없음 -->
        ...
        <DIV style='text-align: left; width: 47%; float: left;'>
          ...
          <FORM name='frm' method='POST' action='./update_file.do' enctype="multipart/form-data">
            <input type="hidden" name="informationno" value="${informationno }">
